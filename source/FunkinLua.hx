...
class FunkinLua {
	...
	public function new(script:String) {
		...
		//achievement lua stuff
		Lua_helper.add_callback(lua, "getAchievement", function(achievementName:String) {
			trace('lua ach1');
			if(!Achievements.isAchievementUnlocked(achievementName))
			{
				trace('lua ach2');
				Achievements.unlockAchievement(achievementName);
				PlayState.instance.startAchievement(achievementName);
			}
		});
		Lua_helper.add_callback(lua, "removeAchievement", function(achievementName:String) {
			trace('lua unach1');
			if(Achievements.isAchievementUnlocked(achievementName))
			{
				trace('lua unach2');
				Achievements.lockAchievement(achievementName);
			}
		});
